...
NETEASE_API_BASE = os.environ.get('NETEASE_API_BASE', 'http://127.0.0.1:55486')
def _derive_netease_port():
    env_port = os.environ.get('NETEASE_API_PORT')
    if env_port and str(env_port).isdigit():
        return int(env_port)
    match = re.search(r':(\d+)', NETEASE_API_BASE)
    if match:
        try:
...
NETEASE_API_PORT = _derive_netease_port()
NETEASE_API_DIR = os.path.abspath(os.path.join(CURRENT_DIR, '..', 'api'))
...
NCM_PROCESS = None
NCM_LOG_HANDLE = None
NODE_MODULES_DIR = os.path.join(NETEASE_API_DIR, 'node_modules')
...
def _find_npm_bin(node_bin=None):
    candidates = [
        shutil.which('npm'),
    ...
    if node_bin:
        ...
        candidates.append(os.path.join(node_dir, 'npm'))
    for cand in candidates:
        if cand and os.path.isfile(cand) and os.access(cand, os.X_OK):
            return cand
    return None
...
def _ensure_node_modules():
    if os.path.isdir(NODE_MODULES_DIR):
        return True
    pkg_json = os.path.join(NETEASE_API_DIR, 'package.json')
    if not os.path.isfile(pkg_json):
        logger.error(f"未找到 package.json，无法安装依赖: {pkg_json}")
        return False
    npm_bin = _find_npm_bin()
    if not npm_bin:
        logger.error("未找到 npm，可执行文件，无法自动安装依赖")
        return False
    use_ci = os.path.isfile(os.path.join(NETEASE_API_DIR, 'package-lock.json'))
    install_cmd = [npm_bin, 'ci', '--omit=dev'] if use_ci else [npm_bin, 'install', '--production']
    try:
        logger.info("检测到缺少 node_modules，尝试自动安装 npm 依赖...")
        ...
        if result.returncode != 0 and use_ci:
            logger.warning(f"npm ci 失败，回退 npm install --production... 输出: {result.stdout}")
            result = subprocess.run(
        ...
        if result.returncode == 0:
            logger.info("npm 依赖已自动安装完成。")
            return True
        logger.error(f"npm 安装依赖失败，退出码 {result.returncode}, 输出: {result.stdout}")
    except Exception as e:
        logger.error(f"自动安装 npm 依赖失败: {e}")
    return False

def _get_api_host_port():
    parsed = urlparse(NETEASE_API_BASE)
    host = parsed.hostname or '127.0.0.1'
    port = parsed.port or NETEASE_API_PORT
    return host, port
...
def _wait_ncm_api_ready(timeout: float = 10.0) -> bool:
    host, port = _get_api_host_port()
    start_ts = time.time()
    while time.time() - start_ts < timeout:
        if NCM_PROCESS and NCM_PROCESS.poll() is not None:
            break
...
def stop_ncm_api():
    global NCM_PROCESS, NCM_LOG_HANDLE
    if NCM_PROCESS and NCM_PROCESS.poll() is None:
        try:
            NCM_PROCESS.terminate()
            try:
                NCM_PROCESS.wait(timeout=5)
            except subprocess.TimeoutExpired:
                NCM_PROCESS.kill()
            logger.info("网易云 API 已停止")
        except Exception as e:
            logger.warning(f"停止网易云 API 失败: {e}")
    NCM_PROCESS = None
    if NCM_LOG_HANDLE:
        try:
            NCM_LOG_HANDLE.close()
        except Exception:
            pass
    NCM_LOG_HANDLE = None

def start_ncm_api():
    """从 Python 进程内启动网易云 API。"""
    global NCM_PROCESS, NCM_LOG_HANDLE
    if NCM_PROCESS and NCM_PROCESS.poll() is not None:
        stop_ncm_api()
    if NCM_PROCESS and NCM_PROCESS.poll() is None:
    ...
    if not node_bin:
        logger.error("未找到 node，可执行文件，网易云 API 无法启动")
        return
    entry_file = os.path.join(NETEASE_API_DIR, 'app.js')
    if not os.path.exists(entry_file):
        logger.error(f"网易云 API 入口未找到: {entry_file}")
        return
    if not _ensure_node_modules():
        logger.error(f"网易云 API 依赖未安装或安装失败: {NODE_MODULES_DIR}")
        return
    env = os.environ.copy()
    env.setdefault('PORT', str(NETEASE_API_PORT))
    env.setdefault('NETEASE_API_BASE', NETEASE_API_BASE)
    env.setdefault('NETEASE_DOWNLOAD_PATH', NETEASE_DOWNLOAD_DIR)
    try:
        try:
            NCM_LOG_HANDLE = open(log_file, 'a', buffering=1, encoding='utf-8')
        except Exception:
            NCM_LOG_HANDLE = None
        NCM_PROCESS = subprocess.Popen(
            ...
            cwd=NETEASE_API_DIR,
            env=env,
            stdout=NCM_LOG_HANDLE or subprocess.DEVNULL,
            stderr=NCM_LOG_HANDLE or subprocess.DEVNULL
        )
        logger.info(f"网易云 API 已启动，端口 {NETEASE_API_PORT}，pid={NCM_PROCESS.pid}")
        if not _wait_ncm_api_ready():
            if NCM_PROCESS and NCM_PROCESS.poll() is not None:
                logger.warning(f"网易云 API 进程退出，code={NCM_PROCESS.poll()}")
            logger.warning("网易云 API 未检测到监听端口，可能启动失败或端口被占用")
    except Exception as e:
        logger.error(f"网易云 API 启动失败: {e}")
        stop_ncm_api()

atexit.register(stop_ncm_api)
...
def load_netease_config():
    ...
    if os.path.exists(NETEASE_CONFIG_PATH):
        ...
        # 首次保存默认配置，方便用户查看
        save_netease_config(NETEASE_DOWNLOAD_DIR)

def save_netease_config(download_dir: str):
    global NETEASE_DOWNLOAD_DIR
    NETEASE_DOWNLOAD_DIR = download_dir
    try:
        with open(NETEASE_CONFIG_PATH, 'w', encoding='utf-8') as f:
            json.dump({'download_dir': download_dir}, f, ensure_ascii=False, indent=2)
    except Exception as e:
...
def netease_config():
    ...
    try:
        if request.method == 'GET':
            return jsonify({'success': True, 'download_dir': NETEASE_DOWNLOAD_DIR})
        data = request.json or {}
        target_dir = data.get('download_dir')
        if not target_dir:
            return jsonify({'success': False, 'error': '缺少下载目录'})
        target_dir = os.path.abspath(target_dir)
        os.makedirs(target_dir, exist_ok=True)
        save_netease_config(target_dir)
        return jsonify({'success': True, 'download_dir': target_dir})
    except Exception as e:
